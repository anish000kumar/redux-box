...
If a component needs to pass runtime arguments to a selector, mark it as a
dynamic selector. `connectStore` will pass it through as a callable prop instead
of executing it while mapping state:

```js
import { dynamicSelector } from 'redux-box';

const getPostById = dynamicSelector((state, props, id) =>
...
Modules also expose `dynamicSelect`, which receives the module slice as the
first argument:

```js
const getPostById = postsModule.dynamicSelect((state, id) =>
  state.items.find(post => post.id === id),
);
```
