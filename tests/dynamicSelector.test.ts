...
describe('dynamicSelector', () => {
  it('should mark a selector as dynamic', () => {
    const selector = dynamicSelector(
      (state: { items: string[] }, props: unknown, id: number) => state.items[id]
    );
  ...
  it('should reject non-function selectors', () => {
    expect(() => dynamicSelector({} as any)).toThrow(
      'dynamicSelector expects a function'
    );
  });
});
